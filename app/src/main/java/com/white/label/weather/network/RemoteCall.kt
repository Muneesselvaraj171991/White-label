...
class RemoteCall private constructor() {
    ...
    fun weatherApi(result: Result, latitude:Double, longitude: Double) {
        try {
            val urlConnection = URL("$BASE_URL?latitude=$latitude&longitude=$longitude&hourly=temperature_2m&daily=temperature_2m_max,temperature_2m_min&current_weather=true&timezone=GMT").openConnection() as HttpURLConnection
